...
async function main() {
  ...
  console.log(`  SSL: ${await config.get('app', 'ssl')}`);
  ...
  console.log(`  SSL: ${await config.get('database', 'ssl')}`);
  console.log(`  Max Connections: ${await config.get('database', 'maxConnections')}`);
  ...
  console.log(
    '  EXAMPLE_APP_PORT=8080                  # With EXAMPLE_APP_ prefix'
  );
  console.log(
    '  EXAMPLE_DATABASE_HOST=prod-db...   # With EXAMPLE_DATABASE_ prefix'
  );

  console.log('\n=== Validation ===');
  console.log('You can validate all configuration at startup:');
  console.log(
    '  config.validate();                # Throws if any value is invalid'
  );
  console.log(
    '  const errors = config.getValidationErrors(); # Returns all errors'
  );
