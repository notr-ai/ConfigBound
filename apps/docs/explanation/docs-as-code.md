...
Docs colocated with the code ship with every change. An external wiki can't make the same guarantee.
...
This isn't about eliminating human authorship. Some content can be derived from code; some requires a human to write. The closer docs are to the code they describe, the more accurate they stay. Writing kept with the code can't silently drift. If you change the code, you're already looking at the description. Writing in a separate wiki goes stale without anyone noticing.
...
A Docs as Code project still needs someone to care about the documentation. The principle reduces the cost of keeping factual content accurate; it doesn't make documentation automatic.
